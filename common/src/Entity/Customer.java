...
public class Customer extends User implements Serializable, SQLReady{
	...
	public Customer(String username, String password, String firstname, String lastname, String email, int customerID, CustomerType customerType, String creditCard) {
	...
	public int createNewAddSqlStatement(Connection conn) {
		...
		if(custID > -1) {
			...
			try {
				...
				if(rs.next()) {
					//String qry;
					qry = "INSERT INTO Customer (CustomerID, CustomerType, PurchasePlan, CreditCard, PhoneNumber, CompanyID)" + " VALUES (?,?,?,?,?,?)";
					try {
